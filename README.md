...
服务器部署（Xvfb + noVNC + Web 控制台 + GitHub Actions 自动构建镜像）见 **[DOCKER.md](./DOCKER.md)**。

快捷：

\\ash
cp config.example.json data/config.json   # 编辑邮箱/代理
cp .env.docker.example .env               # WEB_TOKEN / VNC_PASSWORD
docker compose up -d --build
# 控制台 http://IP:8080  |  看浏览器 http://IP:6080/vnc.html
\
---
